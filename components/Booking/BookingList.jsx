...
import { Calendar, CreditCard, MapPin, MessageCircle, X } from 'lucide-react-native';
  ...
  return bookings
    ...
    .map((booking) => {
      ...
      return {
        ...
        renterName: `${booking.guest?.firstName || 'Unknown'} ${booking.guest?.lastName || 'Guest'}`,
        status: statusMap[booking.status.toLowerCase()] || booking.status.toLowerCase(),
        pickupDate: booking.startDate ? formatDate(booking.startDate) : 'N/A',
        returnDate: booking.endDate ? formatDate(booking.endDate) : 'N/A',
        pickupLocation: booking.pickupLocation?.split('+*+')[2] || 'Unknown Location',
...
const InspectionForm = ({ bookingId, visible, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    translateY.value = visible ? withSpring(0, { damping: 20 }) : withSpring(1000);
  }, [visible]);
  ...
  const handleSubmit = () => {
    ...
    onSubmit(bookingId, { fuelLevel: parseInt(fuelLevel), mileage: parseInt(mileage), photo });
    onClose();
  };
  ...
  return (
    <Animated.View style={[styles.inspectionContainer, animatedStyle]}>
      ...
      <TextInput
        ...
        keyboardType="numeric"
      />
      <Button title="Upload Photo" onPress={() => alert('Photo upload functionality to be implemented')} />
...
const BookingList = ({ onOpenChat }) => {
              ...
              inspections: booking.inspections.map((inspection) =>
                inspection.id === inspectionId
                  ? {
                      ...
                      approved: true,
                      approvedById:
                        inspection.submittedByRole === 'GUEST' ? booking.hostId : user.id,
                      approvedByRole: inspection.submittedByRole === 'GUEST' ? 'HOST' : 'GUEST',
                      updatedAt: new Date().toISOString(),
                    }
                  : inspection
              ),
  ...
  const handleCreateDropOffInspection = (bookingId, { fuelLevel, mileage, photo }) => {
    setBookings((prevBookings) =>
      prevBookings.map((booking) =>
        booking.id === bookingId
          ? {
      ...
      >
        <Animated.View style={[styles.card, animatedStyle]}>
            ...
            <View style={styles.detailsContainer}>
              ...
              {booking.inspections.map((inspection) => (
                <View key={inspection.id} style={styles.detailRow}>
                  <Text style={styles.detailText}>
                    {inspection.type}: {inspection.approved ? 'Approved' : 'Pending'} (Submitted by {inspection.submittedByRole})
                  </Text>
                  {!inspection.approved &&
                    ((inspection.submittedByRole === 'HOST' && user.id === booking.guestId) ||
                      (inspection.submittedByRole === 'GUEST' && user.id === booking.hostId)) && (
                      <TouchableOpacity
                        style={[styles.actionButton, styles.payButton]}
                        onPress={() => handleApproveInspection(booking.id, inspection.id)}
                      >
                        <Text style={styles.buttonTextWhite}>Approve {inspection.type}</Text>
                      </TouchableOpacity>
                    )}
                </View>
              ))}
          ...
          {canCreateDropOff && (
            ...
            >
              <Text style={styles.buttonTextWhite}>Create Drop-Off Inspection</Text>
            </TouchableOpacity>
          )}
          ...
          <View style={styles.actionButtons}>
            ...
            <TouchableOpacity
              style={[styles.actionButton, styles.payButton, booking.status !== 'approved' && { opacity: 0.5 }]}
              onPress={(e) => {
                e.stopPropagation();
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <BookingCard
              ...
              onPay={handlePay}
              onPress={(booking) => router.push({ pathname: '/BookingDetail', params: { id: booking.id } })}
            />
          )}
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtitle}>Your upcoming bookings will appear here</Text>
            </View>
          }
        />
      )}
